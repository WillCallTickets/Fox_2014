...
namespace Wcss
{
    public class _DatabaseCommandHelper
    {
        ...
        public List<ListItem> LoadListItemList(string firstColumnName, string secondColumnName)
        {
            ...
            {
                while (dr.Read())
                    list.Add(new ListItem(dr.GetValue(dr.GetOrdinal(firstColumnName)).ToString(), dr.GetValue(dr.GetOrdinal(secondColumnName)).ToString()));
