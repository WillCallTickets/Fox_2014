...
namespace Wcss
{
    ...
    {
        //private StringBuilder fm = new StringBuilder();
            ...
            get
            {
                if (_listing == null)
                {
                    string title = string.Format("{0} {1}", Entity.Display.Date_NoMarkup_StatusNotFirstNoMarkup_NoTime,
                        Entity.ShowRecord.GetShowVenueMainActPart);
            ...
            get
            {
                ...
                if (_billing == null)
                    _billing = string.Format("{0} {1}", this.Date_NoMarkup_StatusNotFirstNoMarkup_NoTime,
                        (Entity.MenuBilling != null && Entity.MenuBilling.Trim().Length > 0) ?
                        Entity.MenuBilling.Trim().ToUpper() : Entity.Display.Heads_NoFeatures);
        ...
        private string fmAllActs(bool includeMarkup, bool verboseHeads, bool includeLinksHeads, bool verboseOpens, bool includeLinksOpens)
        {
            ...
            return string.Format("{0}{1}{2}{3}", (includeMarkup) ? "<div class=\"show-acts\">" : string.Empty,
        ...
        private string _ticketboilerplate = null;
        public string TicketBoilerplate
        {
            get
            {
                if (_ticketboilerplate == null)
                {
                    StringBuilder sp = new StringBuilder();
                    bool includeMarkup = true;
                    ShowDate sd = Entity;

                    //dont display if show is in the past - show info for a week?
                    if (sd.DateOfShow > _Config.SHOWOFFSETDATE.AddDays(-7))
                    {
                        //default ticket texts
                        string ttDefault = _Config._TicketingText_Default;
                        string ttOver21 = _Config._TicketingText_TwentyOnePlus;
                        string ttAllAges = _Config._TicketingText_AllAges;

                        if (ttDefault.Length > 0 || ttOver21.Length > 0 || ttAllAges.Length > 0)
                        {
                            if (includeMarkup)
                                sp.AppendFormat("<div class=\"event-ticket-boilerplate\">");

                            if (ttDefault.Length > 0)
                                sp.AppendFormat("<div>{0}</div>", ttDefault);

                            string ages = sd.AgesString;

                            if ((ages.ToLower().IndexOf("twenty one") != -1 || ages.ToLower().IndexOf("21+") != -1) && ttOver21.Length > 0)
                                sp.AppendFormat("<div>{0}</div>", ttOver21);

                            if (ages.ToLower().IndexOf("all ages") != -1 && ttAllAges.Length > 0)
                                sp.AppendFormat("<div>{0}</div>", ttAllAges);
                    ...
                    _ticketboilerplate = sp.ToString();
                }

                return _ticketboilerplate;
            }
        }
        ...
        public string BuyLink
        {
            get
            {
                //first two cases are handled in the caller
                if (Entity.DateOfShow < _Config.SHOWOFFSETDATE)
                {
                    //do nothing
                    return null;
                }
                else if (Entity.IsSoldOut || Entity.ShowRecord.IsVirtuallySoldOut)
                {
                    //do nothing
                    return null;
                }
                else if (Entity.ShowRecord.DateOnSale > DateTime.Now)
                {
                    return string.Format("<div class=\"status-future-sale\">Tickets on sale <div>{0}</div></div><br/>",
                        Entity.ShowRecord.DateOnSale.ToString("ddd MMM dd"));

                    //return string.Format("<div class=\"status-future-sale\">Tickets on sale {0} {1}</div><br/>",
                    //    Entity.ShowRecord.DateOnSale.ToString("MM/dd/yyyy hh:mmtt"), _Config._TimeZone_Display);
                }
                else
                {
                    string _buylink = null;
                    List<ShowDate> list = new List<ShowDate>();
                    list.AddRange(Entity.ShowRecord.ShowDateRecords().GetList().FindAll(delegate(ShowDate match)
                    {
                        return (match.IsActive && match.ShowStatusRecord.Name == _Enums.ShowDateStatus.OnSale.ToString() &&
                            match.TicketUrl != null && match.TicketUrl.Trim().Length > 0 && (!match.IsSoldOut) && (!match.ShowRecord.IsVirtuallySoldOut));
                    }));
                    int dateCount = list.Count;

                    if (Entity.TicketUrl != null && Entity.TicketUrl.Trim().Length > 0)
                    {
                        //_buylink = string.Format("<a{0} class=\"btn btn-success\" href=\"{1}\">{2}Buy Tickets!{3}{2}{4}{3}</a>",
                        //_buylink = string.Format("<a{0} class=\"btn\" href=\"{1}\">{2}Buy Tickets!{3}{2}{4}{3}</a>",
                        //_buylink = string.Format("<a{0} class=\"status-buy\" href=\"{1}\">{2}Buy Tickets!{3}{2}{4}{3}</a>",
                        _buylink = string.Format("<a{0} class=\"status-buy\" href=\"{1}\">{2}Buy Tickets!{3}{2}{4}{3}</a>",
                            //_buylink = string.Format("<div class=\"buy-ticket\">{0}Buy Tickets!{1}{0}{2}{1}</div>",
                            (_Config._ExternalTicketUrl_OpensNewWindow) ? " target=\"_blank\"" : string.Empty,
                            Entity.TicketUrl.Trim(),
                            (dateCount > 1) ? "<span>" : string.Empty,
                            (dateCount > 1) ? "</span>" : string.Empty,
                            (dateCount > 1) ? Entity.DateOfShow.ToString() : string.Empty
                            );
                    }
