...
namespace Wcss
{
    public partial class JShowAct : Utils._Collection.IDisplayOrderable
    {
        ...
        {
            return Utils.ParseHelper.ReplaceExtraWhitespaces(string.Format("{0} {1} {2} {3} {4}", 
                this.PreText ?? string.Empty,
                this.ActRecord.Name_Displayable, 
                this.ActText ?? string.Empty, 
                this.Featuring ?? string.Empty, 
        ...
        public string SimpleDisplayName
        {
            get
            {
                return string.Format("{0}{1}{2}",
                    this.ShowDateRecord.DateOfShow.ToString("MM/dd/yy"),
                    (this.TopBilling_Effective) ? string.Format(" <span class=\"glyphicon glyphicon-star\"></span> ") : " ",
                    this.ShowDateRecord.ShowRecord.GetShowVenueMainActPart);
            }
        }
    }
