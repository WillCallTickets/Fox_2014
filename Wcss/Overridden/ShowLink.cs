...
namespace Wcss
{
    ...
    /// </summary>
    public partial class ShowLink : Utils._Collection.IDisplayOrderable
    {
        ...
        public string LinkUrl_BaseLink
        {
            get
            {
                if (_baseLink == null)
                {
                    _baseLink = string.Empty;

                    if (IsShowLink)
                    {
                        Show s = new Show(int.Parse(this.LinkUrl));
                        if (s != null)
                            _baseLink = string.Format("/{0}", s.FirstShowDate.ConfiguredUrl);
                    }
                    else
                        _baseLink = Utils.ParseHelper.FormatUrlFromString(this.LinkUrl);
                }
