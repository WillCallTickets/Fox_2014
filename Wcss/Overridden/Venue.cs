...
namespace Wcss
{
    public partial class Venue : _AlphaOrderedEntity.IAlphaOrderedEntity
    {
        [XmlAttribute("Capacity")]
        public int Capacity
        {
            get { return (!this.ICapacity.HasValue) ? 0 : this.ICapacity.Value; }
            set { this.ICapacity = (value <= 0) ? (int?)null : value; }
        }
        ...
        public string listInfo
        {
            get
            {
                return System.Text.RegularExpressions.Regex.Replace(string.Format("{0} {1} {2} {3}", ShortAddress ?? string.Empty,
                    (BoxOfficePhone != null) ? Utils.ParseHelper.FormatPhoneNumberWithDecimal(BoxOfficePhone) : string.Empty,
        ...
        private string path_original
        {
            get
            {
                return (this.PictureUrl != null && this.PictureUrl.Trim().Length > 0) ?
                    string.Format("{0}{1}", _Config._VenueImageStorage_Local, this.PictureUrl) : string.Empty;
            }
        }
