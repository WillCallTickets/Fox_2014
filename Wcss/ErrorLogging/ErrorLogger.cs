    ...
    public partial class ErrorLogger
    {
        ...
        public static int WriteErrorToLog(Exception ex)
        {
            ...
            else
            {
                if (ctx.Request != null)
                {
                    string host = ctx.Request.Url.Host.ToLower();

                    if (host.IndexOf("fox") != -1)
                        _form = Wcss._Enums.Principal.fox.ToString();
                    else if (host.IndexOf("bt.") != -1 || host.IndexOf("boulder") != -1 || host.IndexOf("bowl") != -1)
                        _form = Wcss._Enums.Principal.bt.ToString();
                    else if (host.IndexOf("z2.") != -1 || host.IndexOf("z2ent") != -1)
                        _form = Wcss._Enums.Principal.z2.ToString();

                    _form += string.Format(" - {0}", ctx.Request.Url.ToString().ToLower());
                }
                else
                    _form = "no request";
            }
