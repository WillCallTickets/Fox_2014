...
{
   public partial class Subscriptions : MainBaseControl
   {
      ...
      protected void chkSubs_DataBinding(object sender, EventArgs e)
      {
          ...
          if (_Common.IsAuthdAdminUser())//admin users
          {
      ...
      public void SaveSubscriptions()
      {
          ...
          if(usr != null)
          {
              foreach (ListItem li in chkSubs.Items)
              {
                  ...
                  SubscriptionUser sub = usr.SubscriptionUserRecords().GetList().Find(
                      delegate(SubscriptionUser match) { return (match.TSubscriptionId.ToString() == li.Value); } );
